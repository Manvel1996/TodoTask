...
export default function TodoBody({setList,setDeleted,setRemoveId}) {
  ...
  return (
    <div className='todoBody' key={uuidv4()}>
        {
           localStorage.getItem("userList")?  localStorage.getItem("userList").length && JSON.parse(localStorage.getItem("userList")).map(el=>{
            return(
                <div key={uuidv4()} className='todoBodyList' id = {el.hiddened?"displayHidden":""}>
                    <input type="checkbox" className = "checkbox" checked={el.isComplited} onChange={(event)=>{
                    ...
                    <p className={el.isComplited?"opacityTask":null} id="taskTag">{el.text}</p>
                    <button onClick={()=>{
                        setDeleted(true)
                        setRemoveId(el.id)
                    }}>X</button>                
